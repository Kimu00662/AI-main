...
private static class ApiEndpoint {
    ...
    void onFailure() {
        ...
        callCount = 0;
        // 失败同样消耗该 API 槽位的 1 次权重。
        slotCallCounts.merge(slot, 1, Integer::sum);
    }

    boolean needRotate() {
        return slotCallCounts.getOrDefault(slot, 0) >= weight;
    }
...
if (reasoningEffort == null || reasoningEffort.isEmpty()) {
    ...
    if (reasoningEffort == null) reasoningEffort = "default";
        ...
        // 同一 API 槽位的多个模型共享该 API 的总权重
        int perWeight = Math.max(1, weight);
...
+ "9. 【说话人识别死命令】\n"
+ "   - 上下文中以“我：”开头的每一行，都是用户自己发过的话，绝不是对方说的。\n"
+ "   - 以“对方：”开头的每一行，才是对方说过的话。\n"
+ "   - 推断回复目标时，只有上下文中存在“对方：”开头的行，才能写“回复对方这句话”，且引用的原话必须来自“对方：”那一行；绝不能把“我：”那一行当成对方原话。\n"
+ "   - 如果上下文中最后一条消息是“我：”开头（说明用户刚发过消息、对方尚未回复），应优先判断用户是在“补充自己的历史消息”，而不是“回复对方”。\n"
...
private static synchronized String executeRequestWithRotation(JSONObject body, OkHttpClient forceClient, boolean isReceive) throws IOException {
    ...
    while (true) {
        ...
        /*
         * ===== 新的“轮次”定义 =====
         * slotCallCounts 是“本轮该 API 已经实际发出的请求次数”。
         * 成功 + 失败都算 1 次。
         *
         * 最重要的一点：API 权重用完时，只标记该 API 本轮完成，绝不把它的计数
         * 立刻清零。只有所有适用 API 都完成本轮后，才整体开启下一轮。
         * 因此：API1 权重1失败 -> API2权重1成功 -> 不会重新给API2权重，
         * 必须等API1冷却结束后才进入下一轮。
         */
        boolean allWeightsExhausted = true;
        long earliestCooldown = Long.MAX_VALUE;
        long now = System.currentTimeMillis();
        for (Integer sid : slotIds) {
            List<ApiEndpoint> list = slots.get(sid);
            if (list == null || list.isEmpty()) continue;
            int weight = Math.max(1, list.get(0).weight);
            int used = slotCallCounts.getOrDefault(sid, 0);
            if (used < weight) allWeightsExhausted = false;
            for (ApiEndpoint ep : list) {
                if (ep.cooldownUntil > now) {
                    earliestCooldown = Math.min(earliestCooldown, ep.cooldownUntil);
                }
            }
        }

        if (allWeightsExhausted) {
            // 如果本轮所有 API 都已用完，但还有失败 API 在冷却：绝不能重置权重。
            // 等冷却结束后，再整体开启下一轮。
            if (earliestCooldown != Long.MAX_VALUE) {
                long waitMs = Math.max(1L, earliestCooldown - System.currentTimeMillis());
                try {
                    Thread.sleep(Math.min(API_COOLDOWN_MS, waitMs));
                } catch (InterruptedException e) {
                    Thread.currentThread().interrupt();
                    throw new IOException("API 调度被中断", e);
                }
                continue;
            }
        ...
        int slotWeight = Math.max(1, candidates.get(0).weight);
        int usedCalls = slotCallCounts.getOrDefault(slotId, 0);
        ...
        /*
         * ===== 模型轮换 =====
         * 一个 API 有 2~6 个模型时，模型是“选择池”，不是额外权重。
         * 例如 API2：权重1 + 模型A/B：
         *   第1轮/第1次 -> A
         *   下一轮再次轮到 API2 -> B
         *   再下一轮 -> A
         *
         * 即每次 API 实际消耗 1 个权重，同时模型游标向后移动 1 格。
         */
        int cursor = slotModelCursor.getOrDefault(slotId, 0);
        if (cursor < 0 || cursor >= candidates.size()) cursor = 0;

        ApiEndpoint targetEp = null;
        long modelEarliestCooldown = Long.MAX_VALUE;
        for (int offset = 0; offset < candidates.size(); offset++) {
            int idx = (cursor + offset) % candidates.size();
            ApiEndpoint c = candidates.get(idx);
            ...
            if (c.cooldownUntil > 0) {
                modelEarliestCooldown = Math.min(modelEarliestCooldown, c.cooldownUntil);
            }
        }

        // 当前 API 暂时没有可用模型：让其它还有剩余权重的 API 先工作。
        if (targetEp == null) {
            boolean moved = false;
            for (int step = 1; step <= slotIds.size(); step++) {
                int otherIndex = (slotIndex + step) % slotIds.size();
                int otherSlot = slotIds.get(otherIndex);
                List<ApiEndpoint> otherCandidates = slots.get(otherSlot);
                int otherWeight = Math.max(1, otherCandidates.get(0).weight);
                int otherUsed = slotCallCounts.getOrDefault(otherSlot, 0);
                if (otherUsed >= otherWeight) continue;

                boolean otherAvailable = false;
                for (ApiEndpoint c : otherCandidates) {
                    if (c.isAvailable()) {
                        otherAvailable = true;
                        break;
                    }
            ...
            long waitMs = modelEarliestCooldown == Long.MAX_VALUE
                    ? API_COOLDOWN_MS
                    : Math.max(1L, modelEarliestCooldown - System.currentTimeMillis());
            try {
                Thread.sleep(Math.min(API_COOLDOWN_MS, waitMs));
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
        ...
        try {
            ...
            // 失败也消耗 1 次 API 权重，并让失败模型进入 3 秒冷却。
            targetEp.onFailure();
            Log.w(TAG, "HT_AI 端点 " + targetEp.model + " 失败，冷却" + (API_COOLDOWN_MS / 1000) + "秒: "
